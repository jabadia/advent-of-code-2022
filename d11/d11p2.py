...
class Monkey:
    ...
    def inspect(self, item, other_monkeys, divisor):
        self.items_inspected += 1
        self.items.remove(item)
        old = item
        item = eval(self.operation.split(' = ')[1]) % divisor
        # item = item // 3
    ...
    for round in range(10000):
        for monkey in monkeys:
            for item in list(monkey.items):
                monkey.inspect(item, monkeys, divisor)
        # print([monkey.items_inspected for monkey in monkeys])
        pass
        # state = [monkey.items for monkey in monkeys]
        # print(state)
